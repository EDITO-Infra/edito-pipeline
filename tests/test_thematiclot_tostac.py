...
def test_layers_to_process(pipeline_config):
    ...
    lots_to_process = pipeline_config['select'][0]['thematic_lot']
    ...
    for id, layer in layer_catalog.layer_catalog_dict.items():
        assert layer['thematic_lot'] in lots_to_process, "Layer ID should be in layers to process"
...
def test_transfer_native_asset(layer: ('Layer'), layer_catalog_dict: ('dict')):
    ...
    if response.status_code == 200:
        logging.info("native asset is downloadable")
    elif S3Utils.is_subsettable_arco_asset(layer.metadata['native_asset']):
        logging.info("native asset is not downloadable but should be on s3")
...
def test_convert_arco(layer: ('Layer')):
    if 'attributes' in layer.metadata:
        ...
        logging.info("no attributes to use for arco conversion")
